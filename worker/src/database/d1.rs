...
impl D1Backend {
    ...
    pub async fn rename_cache(&self, old: &str, new: &str) -> WorkerResult<RenameOutcome> {
        let taken = self
            .db
            .prepare("SELECT 1 FROM cache WHERE name = ?1 AND deleted_at IS NULL")
            .bind(&[new.into()])
            .map_err(|e| WorkerError::Database(format!("Bind error: {}", e)))?
    ...
    pub async fn purge_deleted_cache(&self, name: &str) -> WorkerResult<()> {
        // Dependents first (D1 does not cascade), cache row last so the subqueries
        // can still resolve its id.
        for sql in [
            "DELETE FROM object WHERE cache_id IN \
             (SELECT id FROM cache WHERE name = ?1 AND deleted_at IS NOT NULL)",
